  ...
  <!-- sno,order id,order date,dealer city,
   qty,gst%,gst amount,total amount,payment mode,payment status,remarks ,attachment,view details -->
      ...
      <?php //
        ...
        // $sql = "SELECT * FROM `tbl_order` where `dealer_id`='$delid' AND `product`='Laptop' || `product`='Desktop' order by id desc";
        $sql = "SELECT sum(qty) as qty1,sum(`gst_amount`) as gst1,sum(`total_price`) as total1,`order_date`,`remarks`,`order_status`,`payment_mode`,`orderid`,`attachment`,`status_date`,`id` FROM `tbl_order` where `dealer_id`='$delid' AND `product`='Laptop' || `product`='Desktop' group BY orderid order by orderid";
      ...
      $res="<tr><td style='text-align:center;'>$s<input type='hidden' id='txtN' name='txtN' value='$crow[id]'></td>";
      
      $res.="<td style='text-align:center;'><a href='order-detail-view.php?odid=".$crow['orderid']."'>".$crow['orderid']."</a></td>
