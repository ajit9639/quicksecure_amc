...
class Invoice{
    ...
    private $user  = 'root';
    private $password   = "kis1601";
    private $database  = "insight_dealer_db";   
	private $invoiceUserTable = 'tbl_invoice_user';	
    private $invoiceOrderTable = 'tbl_invoice_order';
	private $invoiceOrderItemTable = 'tbl_invoice_order_item';
	...
	public function saveInvoice($POST) {		
		$sqlInsert = "
			...
			VALUES ('0', '-', '-', '0', '0', '0', '0', '0', '0', '".$POST['notes']."')";		
		mysqli_query($this->dbConnect, $sqlInsert);
		$lastInsertId = mysqli_insert_id($this->dbConnect);
		for ($i = 0; $i < count($POST['productCode']); $i++) {
			$sqlInsertItem = "
			INSERT INTO ".$this->invoiceOrderItemTable."(order_id, modalno,processor1,ram1,hddssd,os1,graphics1,display1, order_item_quantity, order_item_price, order_item_final_amount) 
			VALUES ('".$lastInsertId."', '".$POST['modelno'][$i]."', '".$POST['processor'][$i]."', '".$POST['ram'][$i]."', '".$POST['hddssd'][$i]."', '".$POST['os'][$i]."', '".$POST['graphics'][$i]."', '".$POST['display'][$i]."', '".$POST['quantity'][$i]."', '".$POST['price'][$i]."', '".$POST['total'][$i]."')";			
